...
我们相信，区块链是一种强大的技术基础设施，可以支持创新的去中心化应用。我们希望通过低代码开发平台，让开发者能够充分利用区块链的优势，而不需要关心底层的技术细节。

我们的平台既适应当前的需求，也展望未来的可能。开发者无需受限于特定的编程语言或区块链平台，只需专注于实现“业务逻辑”。这样，无论是“传统”应用还是复杂的企业软件和 Web2 互联网应用，都能轻松地转变为去中心化的  Web3 应用。
...
* **可视化开发**。这意味着开发人员可以使用一些可视化建模工具来创建和修改领域模型，而不需要编写复杂的代码。如果有必要，他们也可以直接编辑领域模型对应的纯文本文件，也就是说，这些文件需要遵循一种 DSL（领域专用语言）的规范。
...
还有“表达式语言”，虽然想要较完美地实现这个特性可能是复杂的（即使我们的平台想要实现它，优先级可以也比较低）——我们需要开发编译器将以这种表达式语言编写的代码编译为各个链的虚拟机（EVM、MoveVM 等）可以执行的指令，但是有了它，开发者就可以仅使用这门“表达式语言”编写业务逻辑，开发出来的应用直接可以运行在不同的链上。不需要去学习和使用各个链的开发语言，这大大节约了应用的开发和移植成本。
